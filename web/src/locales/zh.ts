...
const zh = {
  ...
  'registryCreds.manageTitle': '{host} 的凭据',
  'registryCreds.manageExplain': '选择启动器从 {host} 拉取镜像时使用的已保存凭据。',
  'registryCreds.manageSave': '保存凭据',
  'migrationDegraded.title': '从 1.x 升级仅恢复了部分数据',
  'migrationDegraded.titlePartial': '升级时部分数据无法读取',
  'migrationDegraded.bodyPartial': '旧数据库已打开，但有 {entries} 条记录和 {subtrees} 个子树无法解码，已在以下位置跳过：{maps}。请检查工作区、命名空间和密钥是否完整。',
  'migrationDegraded.body': '无法读取旧数据库，启动器仅从磁盘重建了可恢复的部分：{workspaces} 个工作区、{namespaces} 个命名空间、{recovered} 个仓库地址。密钥未迁移，需要重新录入镜像仓库和 git 凭据。',
  'migrationDegraded.backup': '您原始的 1.x 数据库保持原样，位于 {path}。',
  'migrationDegraded.reason': '原因：{reason}',
  'registryCreds.saved': '凭据已保存',
  'registryCreds.remove': '解除绑定',
  'registryCreds.removed': '镜像仓库绑定已解除',
  'registryCreds.removeTooltip': '解除当前用于 {host} 的凭据绑定',
  ...
  'workspace.config.rawEdit': '编辑 workspace-v1.yml...',
  ...
  'secretPicker.hostMismatch': '此密钥属于 {host}，而非当前镜像仓库',
  ...
  'config.workspace.title': '工作区',
  'config.workspace.desc': '启动器从该 Git 仓库读取捆绑包、应用和命名空间模板。',
  'config.workspace.settings': '工作区设置...',
  'config.workspace.editTitle': '工作区设置',
  'config.workspace.none': '没有活动的工作区',
  'config.registry.title': '镜像仓库凭据',
  'config.registry.desc': '用于从私有 Docker 镜像仓库拉取镜像的凭据。每个主机绑定一个已保存的密钥。',
  'config.registry.colHost': '仓库主机',
  'config.registry.colSecret': '凭据',
  'config.registry.configure': '配置',
  'config.registry.unbound': '未配置',
  'config.registry.unknownSecret': '找不到密钥：{id}',
  'config.registry.empty': '暂无需要凭据的镜像仓库。',
  ...
  'deps.title': '依赖',
  'deps.col.dependency': '依赖',
  'deps.col.current': '当前',
  'deps.col.available': '可用',
  'deps.col.status': '状态',
  'deps.empty': '该 namespace 没有基础设施依赖',
  'deps.status.upToDate': '已是最新',
  'deps.status.pendingMinor': '{version} 将在下次启动时生效',
  'deps.status.upgradeAvailable': '有可用升级',
  'deps.status.requiresLauncherUpdate': '有可用升级 — 需要更新版本的启动器',
  'deps.upgrade': '升级',
  'deps.updateLauncher': '更新启动器',
  'deps.updateLauncherHint': '更新启动器后即可迁移',
  'deps.updateLauncherCli': '在主机上执行 {cmd} 后即可迁移',
  'deps.rollbackPending': '待回滚：{message}',
  'deps.lastResult.success': '上次迁移：{id} {from} → {to} 成功',
  'deps.lastResult.failed': '上次迁移：{id} {from} → {to} 失败：{error}',
  'deps.banner.available': '有可用的依赖升级：{list}',
  'deps.banner.needLauncher': '以下依赖需要更新版本的启动器：{list}',
  'deps.banner.details': '详情',
  'deps.banner.dismiss': '忽略',
  'deps.confirm.intro': '将 {id} 从 {from} 迁移到 {to}。数据会先导出，再恢复到新的存储卷并校验，然后才切换 namespace。',
  'deps.confirm.start': '开始迁移',
  ...
  'deps.step.pull-image': '正在拉取新镜像',
  'deps.step.start-source': '正在启动当前数据库',
  'deps.step.dump': '正在导出数据',
  'deps.step.stop-source': '正在停止当前数据库',
  'deps.step.create-volume': '正在创建新存储卷',
  'deps.step.start-target': '正在启动新数据库',
  'deps.step.restore': '正在恢复数据',
  'deps.step.verify': '正在校验',
  'deps.step.stop-target': '正在停止新数据库',
  'deps.result.success': '{id} 迁移成功',
  'deps.result.oldVolume': '之前的数据保留在存储卷 {volume} 中；不再需要时可在存储卷页面删除',
  'deps.result.failed': '{id} 迁移失败',
  'deps.result.rolledBack': '所有更改已回滚；namespace 仍运行在之前的版本上',
  'deps.controls.migrating': '正在进行依赖迁移',
  'deps.status.blocked': '升级被阻止',
  'deps.status.bundleOlder': '捆绑包提供的版本更旧',
  'deps.banner.blocked': '厂商不允许该升级：{list}',
  'deps.confirm.introCopy': '将 {id} 从 {from} 升级到 {to}。先复制数据卷，再在副本上完成升级并校验，之后才把命名空间切换过去。',
  'deps.zk.note': 'ZooKeeper 3.8 与 3.9 使用相同的磁盘数据格式，因此这次升级只是更换容器。启动器仍会先复制数据，以免将来格式变更时丢失协调状态。',
  'deps.step.copy-volume': '正在复制数据卷',
  'deps.step.start-old': '正在副本上启动当前版本',
  'deps.step.pre-upgrade': '正在为新版本准备数据',
  'deps.step.stop-old': '正在停止当前版本',
  'deps.step.start-new': '正在启动新版本',
  'deps.step.post-upgrade': '正在新版本上完成升级',
  'deps.step.stop-new': '正在停止新版本',
  'deps.step.switch-generation': '正在切换回上一个数据卷',
  'deps.step.start-namespace': '正在启动命名空间',
  'deps.progress.rollbackTitle': '正在回滚 {id}',
  'deps.rollback.action': '回滚到 {version}',
  'deps.rollback.field.version': '版本',
  'deps.rollback.field.volume': '数据卷',
  'deps.rollback.field.frozen': '将被搁置的卷',
  'deps.rollback.field.migratedAt': '迁移时间',
  'deps.rollback.title': '回滚 {id}',
  'deps.rollback.confirm': '回滚',
  'deps.rollback.unavailable': '无法回滚：{problem}',
  'deps.result.rolledBack.to': '{id} 已回滚到 {to}',
  'deps.result.rolledBackDone': '{id} 已回滚到上一个版本',
  'deps.result.frozenVolume': '较新的数据留在卷 {volume} 中；启动器会保留它并且不再读取',
  'deps.result.rollbackFailed': '{id} 回滚失败',
  'deps.result.rollbackUnchanged': '没有任何改动：该依赖仍运行原先的版本',
  'deps.controls.rollingBack': '正在执行依赖回滚',
  ...
  'namespace.form.bundlesRepoHint': '该列表来自工作区配置 \u2014 请在工作区设置中添加或修改仓库。',
