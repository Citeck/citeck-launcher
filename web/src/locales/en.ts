...
const en = {
  ...
  'registryCreds.manageTitle': 'Credentials for {host}',
  'registryCreds.manageExplain': 'Choose which stored credential the launcher uses to pull images from {host}.',
  'registryCreds.manageSave': 'Save credential',
  'migrationDegraded.title': 'Upgrade from 1.x recovered only part of your data',
  'migrationDegraded.titlePartial': 'Part of your data could not be read during the upgrade',
  'migrationDegraded.bodyPartial': 'The old database opened, but {entries} record(s) and {subtrees} sub-tree(s) could not be decoded and were skipped in: {maps}. Check that your workspaces, namespaces and secrets are complete.',
  'migrationDegraded.body': 'The old database could not be read, so the launcher rebuilt what it could from disk: {workspaces} workspace(s), {namespaces} namespace(s), {recovered} repository URL(s). No secrets were carried over — registry and git credentials must be entered again.',
  'migrationDegraded.backup': 'Your original 1.x database is untouched at {path}.',
  'migrationDegraded.reason': 'Cause: {reason}',
  'registryCreds.saved': 'Credentials saved',
  'registryCreds.remove': 'Remove binding',
  'registryCreds.removed': 'Registry binding removed',
  'registryCreds.removeTooltip': 'Unbind the credential currently used for {host}',
  ...
  'workspace.config.title': 'Workspace configuration',
  'workspace.config.hint': 'Edit workspace-v1.yml. Your changes are saved as an overlay on the git reference and re-applied on every sync.',
  'workspace.config.invalidYaml': 'Invalid YAML: {error}',
  'workspace.config.saved': 'Workspace configuration saved',
  ...
  'workspace.config.rawEdit': 'Edit workspace-v1.yml...',
  ...
  'secretPicker.hostMismatch': 'This secret belongs to {host} — not to this registry',
  ...
  'config.workspace.title': 'Workspace',
  'config.workspace.desc': 'Git repository the launcher reads bundles, applications and namespace templates from.',
  'config.workspace.settings': 'Workspace settings...',
  'config.workspace.editTitle': 'Workspace settings',
  'config.workspace.none': 'No active workspace',
  'config.registry.title': 'Registry credentials',
  'config.registry.desc': 'Credentials used to pull images from private Docker registries. Each host is bound to one stored secret.',
  'config.registry.colHost': 'Registry host',
  'config.registry.colSecret': 'Credential',
  'config.registry.configure': 'Configure',
  'config.registry.unbound': 'Not configured',
  'config.registry.unknownSecret': 'Missing secret: {id}',
  'config.registry.empty': 'No registry hosts require credentials yet.',
  ...
  'logViewer.download.saved': 'Log saved to Downloads',
  'logViewer.download.openFolder': 'Open folder',
  ...
  'imageDetails.platform': 'Platform',
  'imageDetails.created': 'Created',
  'imageDetails.pulling': 'Pulling image…',
  'imageDetails.notPulled': 'Image not pulled yet',
  'imageDetails.pull': 'Pull',
  ...
  'deps.col.dependency': 'Dependency',
  'deps.col.current': 'Current',
  'deps.col.available': 'Available',
  'deps.col.status': 'Status',
  'deps.empty': 'This namespace has no infrastructure dependencies',
  'deps.status.upToDate': 'Up to date',
  'deps.status.pendingMinor': '{version} applies on the next start',
  'deps.status.upgradeAvailable': 'Upgrade available',
  'deps.status.requiresLauncherUpdate': 'Upgrade available — requires a newer launcher',
  'deps.upgrade': 'Upgrade',
  'deps.updateLauncher': 'Update the launcher',
  'deps.updateLauncherHint': 'Update the launcher to migrate',
  'deps.updateLauncherCli': 'Run {cmd} on the host to migrate',
  'deps.rollbackPending': 'Rollback pending: {message}',
  'deps.lastResult.success': 'Last migration: {id} {from} → {to} succeeded',
  'deps.lastResult.failed': 'Last migration: {id} {from} → {to} failed: {error}',
  'deps.banner.available': 'Dependency upgrade available: {list}',
  'deps.banner.needLauncher': 'Newer launcher needed for: {list}',
  'deps.banner.details': 'Details',
  'deps.banner.dismiss': 'Dismiss',
  'deps.confirm.intro': 'Migrate {id} from {from} to {to}. The data is dumped, restored into a new volume and verified before the namespace is switched over.',
  'deps.confirm.start': 'Start migration',
  'deps.preflight.data': 'Data size: {size}',
  'deps.preflight.host': 'Host (dump): need {need}, free {free}',
  'deps.preflight.volume': 'Volumes: need {need}, free {free}',
  'deps.preflight.shared': 'Disk (dump + new cluster): need {need}, free {free}',
  'deps.preflight.willStop': 'The namespace will be stopped and started again',
  ...
  'deps.step.pull-image': 'Pulling the new image',
  'deps.step.start-source': 'Starting the current database',
  'deps.step.dump': 'Dumping the data',
  'deps.step.stop-source': 'Stopping the current database',
  'deps.step.create-volume': 'Creating the new volume',
  'deps.step.start-target': 'Starting the new database',
  'deps.step.restore': 'Restoring the data',
  'deps.step.verify': 'Verifying',
  'deps.step.stop-target': 'Stopping the new database',
  'deps.result.success': '{id} migrated successfully',
  'deps.result.oldVolume': 'The previous data is kept in volume {volume}; remove it from the Volumes page when you no longer need it',
  'deps.result.failed': 'Migration of {id} failed',
  'deps.result.rolledBack': 'Everything was rolled back; the namespace runs on the previous version',
  'deps.controls.migrating': 'A dependency migration is running',
  'deps.status.blocked': 'Upgrade blocked',
  'deps.status.bundleOlder': 'The bundle offers an older version',
  'deps.banner.blocked': 'Upgrade blocked by the vendor: {list}',
  'deps.confirm.introCopy': 'Upgrade {id} from {from} to {to}. The data volume is copied, the copy is upgraded and verified, and only then is the namespace switched over.',
  'deps.zk.note': 'ZooKeeper 3.8 and 3.9 share one on-disk format, so this upgrade is a container swap. The launcher still copies the data first, so a future format change cannot cost you the coordination state.',
  'deps.step.copy-volume': 'Copying the data volume',
  'deps.step.start-old': 'Starting the current version on the copy',
  'deps.step.pre-upgrade': 'Preparing the data for the new version',
  'deps.step.stop-old': 'Stopping the current version',
  'deps.step.start-new': 'Starting the new version',
  'deps.step.post-upgrade': 'Completing the upgrade on the new version',
  'deps.step.stop-new': 'Stopping the new version',
  'deps.step.switch-generation': 'Switching to the previous data volume',
  'deps.step.start-namespace': 'Starting the namespace',
  'deps.progress.rollbackTitle': 'Rolling back {id}',
  'deps.rollback.action': 'Roll back to {version}',
  'deps.rollback.field.version': 'Version',
  'deps.rollback.field.volume': 'Data volume',
  'deps.rollback.field.frozen': 'Volume kept aside',
  'deps.rollback.field.migratedAt': 'Migrated on',
  'deps.rollback.title': 'Roll back {id}',
  'deps.rollback.confirm': 'Roll back',
  'deps.rollback.unavailable': 'Cannot roll back: {problem}',
  'deps.result.rolledBack.to': '{id} was rolled back to {to}',
  'deps.result.rolledBackDone': '{id} was rolled back to the previous version',
  'deps.result.frozenVolume': 'The newer data stays in volume {volume}; the launcher keeps it and never reads it again',
  'deps.result.rollbackFailed': 'Rollback of {id} failed',
  'deps.result.rollbackUnchanged': 'Nothing was changed: the dependency still runs the version it did before',
  'deps.controls.rollingBack': 'A dependency rollback is running',
  ...
  'namespace.form.bundlesRepoHint': 'The list comes from the workspace configuration \u2014 add or change repositories in workspace settings.',
