...
export function TabBar() {
  ...
  if (tabs.length <= 1) return null // hide if only dashboard
    ...
    <div className="flex items-center border-b border-border bg-card overflow-x-auto">
      {tabs.map((tab) => {
        ...
        return (
          <div
            key={tab.id}
            className={`group flex items-center gap-1 px-3 py-1 text-xs border-r border-border cursor-pointer shrink-0 ${
              isActive
          ...
          >
            ...
            {tab.id !== 'dashboard' && (
              <button
                type="button"
                className="ml-0.5 p-0.5 rounded hover:bg-muted opacity-0 group-hover:opacity-100"
                onClick={(e) => {
                  e.stopPropagation()
