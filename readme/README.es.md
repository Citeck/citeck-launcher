...
Citeck Launcher es el instalador oficial y gestor de contenedores para la plataforma low-code de BPM/ECM **Citeck**. Un único binario de ~24 MB funciona como herramienta de línea de comandos, demonio en segundo plano y aplicación de escritorio multiplataforma, ejecutando cada servicio de Citeck (Keycloak, PostgreSQL, RabbitMQ y las aplicaciones web de Citeck) como un contenedor de Docker y agrupándolos en espacios de nombres aislados. Qué aplicaciones y versiones se ejecutan lo define un **bundle** (por ejemplo, una edición Community o Enterprise).

[Citeck](https://github.com/Citeck) es una plataforma de código abierto para crear aplicaciones de negocio, que combina los enfoques **no-code, low-code y pro-code** para gestionar contenido y procesos. En la práctica, la usas para **gestionar documentos y registros (ECM), automatizar procesos de negocio y flujos de aprobación con un diseñador BPMN integrado, y crear aplicaciones internas — portales, CRM, gestión de casos — con poco o ningún código**, con cuentas de usuario, roles y permisos incorporados. Es una alternativa autoalojada a las suites propietarias de ECM/BPM, adecuada para todos, desde analistas de negocio hasta desarrolladores.

La edición **Community** es totalmente de código abierto y gratuita — cubre la funcionalidad principal de la plataforma y está diseñada para ser compatible con extensiones de cualquier tipo. Para configuraciones más exigentes, la edición **Enterprise** comercial añade soporte profesional y funciones empresariales adicionales. Este launcher instala cualquiera de las dos ediciones. Para preguntas o una consulta, [ponte en contacto con el equipo de Citeck](https://www.citeck.ru/contacts/).
...
| Interfaz | Ventana nativa de la aplicación (GUI) | Terminal — CLI `citeck` + asistente de configuración (TUI) |
...
**Requisitos:** Docker; unos **16 GB** de RAM para la edición Community (**24 GB** para los ~24 servicios de Enterprise); y decenas de GB de disco para las imágenes.
...
> **Para un servidor o VM Linux** — ejecuta estos pasos en el servidor, por SSH.
...
El daemon en modo servidor gestiona contenedores Docker, por lo que su API equivale en la práctica a **privilegios de root** en el host (por ejemplo, `citeck exec` ejecuta comandos dentro de los contenedores). La CLI se comunica con el daemon a través de un socket Unix restringido al usuario del daemon (modo 0600). Con la Web UI habilitada, el daemon también escucha en un puerto TCP: un bind en localhost sirve la API completa protegida únicamente contra CSRF de navegador — eso **no** es autenticación, así que cualquier usuario o proceso local con acceso al puerto obtiene control total; los binds fuera de localhost requieren certificados de cliente mTLS. **En modo servidor la Web UI está deshabilitada por defecto** (la interfaz de servidor compatible es la CLI/TUI). Habilítela de forma deliberada con `server.webui.enabled: true` en `daemon.yml` — y solo en un **host de un único inquilino (single-tenant)** cuyos usuarios locales ya sean de confianza con acceso de nivel Docker/root.

Para cerrar esa brecha en localhost, active la **autenticación por token de API** opcional: establezca `api_auth.enabled: true` en `daemon.yml`. El daemon exigirá entonces `Authorization: Bearer <token>` (o la cookie de sesión del navegador emitida mediante `GET /auth/session?token=…`) en cada petición a `/api` por TCP; el token se toma de `api_auth.token` o se genera automáticamente en `conf/api-token` (modo 0600) al arrancar. Ejecute `citeck ui` para imprimir — y abrir — un enlace de navegador autenticado. Los recursos estáticos de la interfaz siguen siendo públicos; solo se protege la API. El socket Unix (CLI), la aplicación de escritorio y los clientes mTLS no se ven afectados.
