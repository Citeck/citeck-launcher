...
- **Launcher 自身の Web UI は、サーバーモードではデフォルトで無効です** — サポートされるサーバーインターフェースは CLI/TUI です。有効にすると（`daemon.yml` の `server.webui.enabled: true`）、デーモンは TCP ポートでも待ち受けます。localhost にバインドした場合、完全な API がブラウザー CSRF 対策のみで提供されます — これは認証では**ありません**。そのポートに到達できるローカルユーザーやプロセスは誰でも完全な制御権を得ます。有効化は意図的に行い、ローカルユーザー全員に Docker/root 相当のアクセスを任せられると信頼できる**シングルテナントのホスト**でのみ使用してください。localhost 以外へのバインドには mTLS クライアント証明書が必要です。
