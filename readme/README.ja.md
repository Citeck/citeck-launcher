...
| Web UI | 内蔵のネイティブウィンドウ | HTTPS で配信（TLS / Let's Encrypt 対応） |
...
**デスクトップアプリケーション**は、お使いの Windows、macOS、Linux マシン上で Citeck を実行します。同じデーモンと Web UI をネイティブウィンドウ（Wails）でラップしたものです。アプリはデーモンを子プロセスとして監視するため、ウィンドウを閉じてもコンテナは実行され続けます。
...
> **Linux サーバーまたは VM 向け**です（SSH 経由で実行）。お使いのコンピューターでは、[デスクトップアプリ](#デスクトップアプリ)から Citeck を実行してください。
...
- **デスクトップアプリ:** 単独で完結しています。アプリ自身のウィザードと UI からセットアップでき、別途の設定は不要です。
