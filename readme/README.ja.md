...
[Citeck](https://github.com/Citeck) は、独自仕様の ECM/BPM スイートに代わるセルフホスト型のオープンソース low-code プラットフォームです。企業文書に関わるほぼあらゆる業務に利用でき、契約書や購買の承認から、人事プロセス、電子アーカイブ、社内ポータルまで、その用途は多岐にわたります。各プロセスのルートは組み込みの BPMN デザイナーで描き、文書タイプはコードを書かずに設定できます。ユーザー、ロール、権限は標準で組み込まれています。
...
**必要なもの:** Docker · Community 版で **16 GB** RAM、Enterprise 版（約 24 サービス）で **24〜32 GB** · イメージとデータ用に **50 GB 以上**の空きディスク。Windows と macOS では、先に [Docker Desktop](https://www.docker.com/products/docker-desktop/) をインストールしてください。
...
デスクトップアプリケーションは、お使いの Windows、macOS、Linux マシン上で Citeck を実行します。コマンドライン不要の、ふつうのアプリウィンドウです。ウィンドウを閉じても、Citeck はバックグラウンドで動き続けます。
...
20 分ほど経っても停止しているように見える場合は、まず `citeck diagnose`（`--fix` を付けると修復可能なものを自動で修復します）と `citeck logs <app>` から確認してください。
...
- **Namespace（名前空間）** — プラットフォームの分離された 1 つのインスタンス（専用のコンテナ、ボリューム、データを持ちます）。Linux や Kubernetes の名前空間とは関係のない、Launcher 固有の概念です。一般的なサーバーではちょうど 1 つを実行します。
...
`citeck stop <app>` はアプリを**デタッチ**する点に注意してください。`citeck start <app>` を実行するまで、再起動やリロードをまたいで停止したままになります。これは小規模なホストでメモリを解放する方法でもあります — 任意のアプリをいくつかデタッチすれば数 GB を節約できます。
...
- 8 言語にローカライズ済み、bash、zsh、fish、PowerShell 向けのシェル補完付き
...
先にはっきりお伝えしておきます。**サーバーモードのデーモンは Docker を制御するため、その API はホスト上で root 相当のものとして扱ってください**（例えば `citeck exec` はコンテナ内でコマンドを実行します）。だからこそ、安全な選択肢がデフォルトになっています。

- **CLI** は、デーモンのユーザーのみに制限された Unix ソケット（パーミッション 0600）経由でデーモンと通信します。
- **Launcher 自身の Web UI は、サーバーモードではデフォルトで無効です** — サポートされるサーバーインターフェースは CLI/TUI です。有効にすると（`daemon.yml` の `server.webui.enabled: true`）、デーモンは TCP ポートでも待ち受けます。localhost にバインドした場合、完全な API がブラウザー CSRF 対策のみで提供されます — これは認証では**ありません**。そのポートに到達できるローカルユーザーやプロセスは誰でも完全な制御権を得ます。有効化は意図的に行い、ローカルユーザー全員が Docker/root 相当のアクセスを信頼されている**シングルテナントのホスト**でのみ使用してください。localhost 以外へのバインドには mTLS クライアント証明書が必要です。
- **この localhost のギャップを塞ぐには**、API トークン認証を有効にします: `daemon.yml` の `api_auth.enabled: true`。これにより、TCP 経由のすべての `/api` リクエストに `Authorization: Bearer <token>`（または `GET /auth/session?token=…` が発行するブラウザーのセッション Cookie）が必要になります。トークンは `api_auth.token` から取得されるか、起動時に `conf/api-token`（パーミッション 0600）へ自動生成されます。`citeck ui` は認証済みリンクを表示し、そのまま開きます。静的な UI アセットは公開のままで、保護されるのは API のみです。Unix ソケット、デスクトップアプリ、mTLS クライアントには影響しません。

（このセクションは *Launcher* の管理 UI についての説明であり、インストール後にログインする Citeck プラットフォームの UI についてではありません。）
...
ご質問、Enterprise ライセンス、ご相談については、[Citeck チームまでお問い合わせください](https://www.citeck.ru/contacts/)。
