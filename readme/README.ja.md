...
Citeck Launcher は、**Citeck** ローコード BPM/ECM プラットフォーム向けの公式インストーラーおよびコンテナマネージャーです。約 24 MB の単一バイナリが、コマンドラインツール、バックグラウンドデーモン、クロスプラットフォームのデスクトップアプリとして動作し、すべての Citeck サービス（Keycloak、PostgreSQL、RabbitMQ、および Citeck ウェブアプリケーション）を Docker コンテナとして実行し、それらを分離された名前空間にグループ化します。 どのアプリとバージョンを実行するかは **bundle** で決まります（例: Community または Enterprise リリース）。

[Citeck](https://github.com/Citeck) は、ビジネスアプリケーションを構築するためのオープンソースプラットフォームであり、コンテンツとプロセスを管理するために **ノーコード、ローコード、プロコード** のアプローチを組み合わせています。実際の用途としては、**ドキュメントやレコードの管理（ECM）、組み込みの BPMN デザイナーによるビジネスプロセスと承認ワークフローの自動化、そしてポータル、CRM、ケース管理といった社内アプリをほとんどコードを書かずに、あるいはまったく書かずに構築する** ことができ、ユーザーアカウント、ロール、権限が標準で組み込まれています。これは独自仕様の ECM/BPM スイートに代わるセルフホスト型の選択肢であり、ビジネスアナリストから開発者まで、あらゆる人に適しています。

**Community** エディションは完全なオープンソースかつ無償であり、プラットフォームのコア機能を網羅しているほか、あらゆる種類の拡張に適した設計になっています。より要求の高い構成には、商用の **Enterprise** エディションがプロフェッショナルサポートと追加のエンタープライズ機能を提供します。この Launcher はそのどちらのエディションでもインストールできます。ご質問やご相談がありましたら、[Citeck チームまでお問い合わせください](https://www.citeck.ru/contacts/)。
...
**要件:** Docker に加え、それなりの性能を備えたマシンが必要です。フルスタックでは多数のコンテナ（認証、データベース、メッセージブローカー、および Citeck ウェブアプリケーション）が動作します。Community エディションでは概ね **16 GB RAM**（Enterprise の約 24 サービスでは 24〜32 GB）と、イメージ用に数十 GB のディスクを見込んでください。これはフル機能のプラットフォームであり、軽量なツールではありません。
