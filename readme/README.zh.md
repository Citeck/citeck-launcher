...
[Citeck](https://github.com/Citeck) 是一个自托管的开源低代码平台，用于替代专有的 ECM/BPM 套件。几乎任何涉及企业文档的工作，你都能用它来完成——从合同与采购审批，到人力资源流程、电子档案库乃至企业门户。你在内置的 BPMN 设计器中绘制每个流程的流转路线，无需编写代码即可配置文档类型；用户、角色和权限均已开箱即用。

Citeck Launcher 是让平台跑起来并持续稳定运行的最简单方式。你只需下载一个约 24 MB 的单一二进制文件——它会安装平台，并通过 Docker 启动各项服务。此后，Launcher 会持续监测服务的健康状况，自动重启任何宕掉的服务，还能让平台升级变得简单而可预期。在你自己的电脑上，它以桌面应用的形式运行；在服务器上，则通过命令行使用。
...
**Community** 版完全开源且免费，涵盖平台的核心功能。商业的 **Enterprise** 版增加了专业支持和额外功能；安装它需要由 Citeck 签发的许可证密钥。两个版本都可以通过本启动器安装。
...
- **服务器模式下，启动器自身的 Web UI 默认处于禁用状态** — 受支持的服务器界面是 CLI/TUI。当你启用它（在 `daemon.yml` 中设置 `server.webui.enabled: true`）后，守护进程还会监听一个 TCP 端口。绑定到 localhost 时，完整 API 仅受浏览器 CSRF 防护——这**不是**身份认证，任何能访问该端口的本地用户或进程都将获得完全控制权。请在深思熟虑后再启用，并且只在**单租户主机**上启用——该主机的所有本地用户都已被信任拥有 Docker/root 级访问权限。绑定到非 localhost 地址则要求 mTLS 客户端证书。
...
由 Go（守护进程 + CLI）和 React（内嵌 Web UI）构建；桌面应用通过 Wails webview 包装同一套 UI。前置条件、构建目标以及完整的本地检查门禁（`make check`）记录在 [AGENTS.md](../AGENTS.md) 中。

## 许可证与联系方式

Citeck Launcher 是基于 **LGPL-3.0** 许可证的开源软件——参见 [LICENSE](../LICENSE)。

如有疑问、需要 Enterprise 许可或希望咨询，请[联系 Citeck 团队](https://www.citeck.ru/contacts/)。
