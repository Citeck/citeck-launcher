...
- **The pre-restart thread dump does NOT use `jcmd`** — the runtime images ship a JRE (`/opt/java/openjdk/bin` has `java jfr jrunscript jwebserver keytool rmiregistry` and nothing else), so the old `jcmd 1 Thread.print` recorded `(jcmd failed: exit=127)` on every restart, and it was wrong twice: **PID 1 is `sh -c /entrypoint.sh`, not the JVM** (java is a child — pid 7 in a measured container), so a JDK image alone would not have fixed it. `captureThreadDump` (`internal/namespace/reconciler.go`) tries two mechanisms: (1) the attach protocol from the host via `internal/jvmattach` — nothing needed inside the container, dump kept out of the app log, Linux-only and uid-gated; (2) SIGQUIT to the JVM **inside** the container, which works everywhere Docker does (incl. macOS/Windows desktop, where the containers sit in a VM the host cannot attach into) but delivers the dump through the container's stdout — so that path widens the log tail to `diagLogTailWithDump` (a 132–225-thread dump is 1500–2500 lines; at 500 the file would hold the dump's middle and neither its start nor the failure). The in-container search matches on `readlink /proc/<pid>/exe`, **never on the cmdline**: the scanning shell's own cmdline contains "java", so a cmdline match returns the shell. Vendoring `jattach` was the alternative — rejected in favour of ~200 lines of stdlib Go with no binary blob to pin, verify and re-license. Pinned by `internal/jvmattach/*_test.go` (fake procfs + fake attach listener) and `reconciler_diagnostics_test.go`.
