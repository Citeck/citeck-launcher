...
class ConfirmDialog(
    ...
    companion object {
        ...
        private fun showPopup(params: ConfirmDialogParams) {
            showDialog(ConfirmDialog(params))
        }

        fun show(message: String, onConfirm: () -> Unit) {
            showPopup(
                ConfirmDialogParams(title = DEFAULT_TITLE, message = message, onConfirm = onConfirm)
            )
        }
