...
# Regenerate the embedded HotSpot attach client. Run this after editing
# CiteckAttach.java — the .class is COMMITTED (the Go build has no javac, and CI
# must not depend on a JDK), and TestAttachClassMatchesSource fails the build if
# the two drift apart.
#
# --release 17 is the compatibility floor: UnixDomainSocketAddress needs Java 16+,
# and a class file must not be newer than the JVM being attached to.
JVM_ATTACH_DIR := internal/appfiles/embedded/jvm

jvm-attach-class:
	@command -v javac >/dev/null || { echo "javac not found (JDK 17+ required)"; exit 1; }
	javac --release 17 -d $(JVM_ATTACH_DIR) $(JVM_ATTACH_DIR)/CiteckAttach.java
	@{ \
		echo '# Generated by `make jvm-attach-class` — do not edit by hand.'; \
		echo '# Guards the committed CiteckAttach.class against the source drifting away from'; \
		echo '# it: the class is committed (no javac in the Go build), so nothing else would'; \
		echo '# notice if someone edited the .java and forgot to regenerate.'; \
		echo '# Verify by hand with: sha256sum -c checksums.sha256'; \
		echo "# Compiled with: $$(javac -version 2>&1) --release 17"; \
		cd $(JVM_ATTACH_DIR) && sha256sum CiteckAttach.java CiteckAttach.class; \
	} > $(JVM_ATTACH_DIR)/checksums.sha256
	@echo "Regenerated $(JVM_ATTACH_DIR)/CiteckAttach.class + checksums.sha256"
