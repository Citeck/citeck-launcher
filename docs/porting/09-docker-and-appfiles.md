...
## 5. 2.x порт — desktop-mode volume scoping (`6fe02d1`)

### 5.1 Server vs desktop volume model

| Mode | Volume backing |
|---|---|
| **Server** | Bind-mount in workspace dir: `<launcher-home>/ws/<wsId>/ns/<nsId>/volumes/<name>/`. Auto-scoped by filesystem layout — plain volume names в `ApplicationDef` work as-is. |
| **Desktop** | Docker **named volumes** (cross-platform; bind-mounts don't work cleanly under Docker Desktop on macOS / Windows). Plain volume names МУСТ be scoped или they collide across namespaces. |

До commit `6fe02d1` desktop mode не scoped plain volume names — `mongo2:/data/db`
в двух namespace ended up as one shared Docker volume → data corruption.
`LabelOrigName` was never written, so the Kotlin volume-lookup pattern
(find volume by original name + namespace label) didn't work.

### 5.2 Fix (item 12)

`internal/docker/volumes.go` теперь mirror'ит Kotlin `DockerApi`:

- `CreateVolume(nsRef, originalName)` — creates Docker volume named
  `citeck_volume_{orig}_{ns}_{ws}` (matches `DockerConstants.getVolumeName`)
  with labels:
  - `citeck.launcher.app.name`
  - `citeck.launcher.workspace`
  - `citeck.launcher.namespace`
  - `citeck.launcher.original-name` (= plain name pre-scoping)
  - `citeck.launcher` = `"true"`
  - `com.docker.compose.project` (= `DockerConstants.getDockerProjectName`)
- `GetVolumeByOriginalName(nsRef, originalName)` — filters by
  `LabelNamespace` + `LabelOrigName`, post-filters by workspace label
  (case-insensitive — Docker normalizes label values inconsistently across
  versions; this matches the Kotlin matcher's behavior).
- `ListVolumesForNamespace(nsRef)` — все volumes для namespace, filter
  `LabelNamespace` + `LabelWorkspace`.
- `VolumeSize(volName)` — uses Docker HTTP API `GET /system/df?type=volume`
  (mirrors Kotlin `getVolumesSize`).

### 5.3 Volumes UI listing path (item 13)

`internal/daemon/routes_volumes.go::handleListVolumes` теперь:

- В **server mode**: reads filesystem `volumesDir` как раньше.
- В **desktop mode** (when `config.IsDesktopMode()` is true): uses Docker
  volume API with label filter `LabelNamespace` + `LabelWorkspace`. Size
  via `/system/df`; delete via Docker `VolumeRemove`.

Без этого Volumes UI в desktop mode был always пустой даже когда Docker
named volumes existed.

### 5.4 Critical: contract preservation

The volume naming pattern (`citeck_volume_{srcName}_{ns}_{ws}`) и label
keys — **byte-exact contract** с Kotlin 1.x (см. `docs/porting/10` §6).
Изменение поломает discovery всех existing v1.x deployments + rollback
к Kotlin (см. `docs/porting/ROLLBACK.md` §2).
