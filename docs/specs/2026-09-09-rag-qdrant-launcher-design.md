# Опциональное подключение citeck-rag и Qdrant в лончер (1.x и 2.x)

Дата: 2026-09-09. Статус: дизайн согласован, ожидает реализации.

## 1. Проблема

`citeck-rag` (enterprise) не разворачивается ни одной версией лончера.
...
* Лончер генерирует вебаппы как пересечение бандла и `webapps` (2.x — `internal/namespace/generator.go:99-113`,
...
Следствие: семантический поиск в UI и RAG-инструменты ассистента на стендах из лончера не работают.

## 2. Требования

1. rag и qdrant доступны в **обеих** версиях лончера — 1.x официально ещё не выведена из эксплуатации.
...
**не проверяются**. Проверяется механика лончера: гейт, регенерация по toggle, порядок старта, env,
