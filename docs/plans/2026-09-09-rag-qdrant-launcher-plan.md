...
> оставлены как есть, потому что документ читается теперь как история решений,
> а не как инструкция. **Не исполнять его повторно.** Две вещи в тексте
> заведомо разошлись с тем, что поехало:
>
> - **Task 3, шаги 4-5** велят писать причину ожидания в `app.StatusText`
>   через `i18n.T` и заводить ключ `app.status.waitingForDeps` в
>   `internal/i18n/locales/*.json`. Это было **сознательно отменено**:
>   рантайм-горутина не имеет читателя и потому не имеет языка, а
>   пакетный `i18n.T` держит одну глобальную локаль и не потокобезопасен.
>   Поехало вместо этого: `AppDto.WaitingFor` несёт пары `{app, status}`,
>   а фразу собирает читатель (веб-UI — из своих локалей, CLI — дописывая
>   имена зависимостей в колонку STATUS).
> - **Task 6** назван «changelog 2.11.0»; фактические релиз-ноты выпущены
>   как 2.12.3.
...
- [ ] **Шаг 4: писать причину в StatusText** — ⚠️ ОТМЕНЕНО, см. статус в начале файла:
  причина едет структурой в `AppDto.WaitingFor`, рантайм ничего не рендерит.
...
- [ ] **Шаг 5: добавить ключ во все локали** — ⚠️ ОТМЕНЕНО вместе с шагом 4:
  ключ `app.status.waitingForDeps` живёт в ассете веб-UI, а не в CLI-локалях демона.
