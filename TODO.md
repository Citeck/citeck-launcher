...
### 12. Прозрачная миграция с Kotlin launcher (v1.x)
Пользователь не должен знать что рантайм сменился. Просто обновил бинарь — всё работает.
- [ ] Desktop: H2→SQLite миграция уже автоматическая. Проверить что namespace/secrets/workspace state переносятся полностью
- [ ] Desktop: старые контейнеры (именование `citeck_{app}_{ns}_{ws}`) — при первом старте v2 переименовать или пересоздать
- [ ] Server: подменить бинарь → `systemctl restart citeck` → namespace.yml читается, контейнеры продолжают работать
- [ ] Server: старые контейнеры (именование `_daemon`) — при первом старте v2 обнаружить по labels и пересоздать с новым именем
- [ ] Secrets: если Kotlin шифровал blob — автоимпорт + шифрование дефолтным паролем (server) / диалог (desktop). Уже работает
- [ ] E2E тест: Kotlin v1.3.x → Go v2.0.0 на реальных данных
