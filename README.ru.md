...
демон и веб-интерфейс в нативном окне (Wails). Установщики прикладываются к каждому
...
К каждому установщику прилагается файл `.sha256` для проверки.

**Установка 2.x поверх старого приложения 1.x — чистое обновление на месте**: старые исполняемые
файлы не остаются, данные сохраняются (миграция 1.x → 2.x выполняется автоматически при первом
запуске). Замену делает сама ОС: `.deb`/`.rpm` сохраняют имя пакета `citeck-launcher`, Windows
`.msi` переиспользует upgrade-код 1.x, а macOS `.app` сохраняет имя бандла.
...
citeck stop [app...] [-d|--detach]        Остановка пространства или одного/нескольких приложений (--detach = в фоне)
citeck restart [app] [-d|--detach]        Перезапуск (по умолчанию ждёт; --detach — в фоне)
...
citeck dump-system-info [--full]          Собрать диагностику в ZIP (status, logs, docker inspect, journalctl)
