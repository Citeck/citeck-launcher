...
// The same exception has to cover a companion the GENERATOR detached (qdrant
// beside a detached rag, stt-sidecar beside a detached ai). The app table
// enables its start button off AppDto.Detached — which reports the effective
// answer, manual OR auto (AppTable.tsx) — so a gate that only consults the
// operator's own set refuses a button it has just offered, on exactly the
// namespace where starting the companion is the point: stopped, with the owner
// about to be run from an IDE.
func TestAttachingAnAutoDetachedCompanionIsAllowedWhileTheNamespaceIsStopped(t *testing.T) {
	store, err := storage.NewSQLiteStore(t.TempDir())
	require.NoError(t, err)
	t.Cleanup(func() { _ = store.Close() })
	...
	rt.InjectAppsForTest(&namespace.AppRuntime{
		Name:   "qdrant",
		Status: namespace.AppStatusStopped,
		Def:    appdef.ApplicationDef{Name: "qdrant"},
	})
	rt.SetAutoDetachedApps(map[string]bool{"qdrant": true})
	require.Empty(t, rt.ManualStoppedApps(), "auto-detach is not the operator's intent")
	...
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/api/v1/apps/qdrant/start", http.NoBody))

	require.Equal(t, http.StatusOK, rec.Code, "body=%s", rec.Body.String())
	assert.False(t, rt.IsAppDetached("qdrant"),
		"the explicit start must lift the auto-detach, or the next generation keeps it down")
}
