...
//
// The set is dynamic — it comes from GenResp.GatingApps, the last generation's
// record of apps whose detach state changes WHICH other apps are generated at
// all (e.g. onlyoffice/AI's upstream wiring on the proxy, AI's stt-sidecar).
// A plain single-app start/stop only moves that one container and leaves
// dependent apps' generated config stale, so toggling a gating app must
// regenerate the whole namespace. Individual generators mark their gating
// apps via NsGenContext.MarkGatingApp; nothing in the daemon needs to know
// their names.
//
...
func regenOnAttachToggle(rt *namespace.Runtime, name string) bool {
	return rt.GatingApps()[name]
}

// regenAfterAttachToggleAsync regenerates the namespace off the request
// goroutine after a gating app (see regenOnAttachToggle) is attached or
...
func (d *Daemon) regenAfterAttachToggleAsync(app, action string) {
	go func() {
		if !d.longOp.TryLock(longOpUpdatePass) {
			//nolint:gosec // G706: app is validated by validateAppName and gated to the runtime's GatingApps set; action is a caller literal
