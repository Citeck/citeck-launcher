...
* Stopping `rag` no longer takes Qdrant down with it. The store stays in the namespace as a stopped app you can start on its own — which is what running `rag` from an IDE against this stand needs — and it never starts by itself, so a RAG that is simply switched off still costs no memory. Stopping `rag` while Qdrant is already running leaves Qdrant running: the launcher does not take down a store you may still be using, so stop it yourself when you want the memory back. Qdrant now also publishes its gRPC port (6334), the port `citeck-rag` actually talks to, so a locally run `rag` reaches the stand's store with no extra configuration.
* The same for `ai` and its STT sidecar: stopping `ai` keeps the sidecar described (and stopped) instead of deleting it, so an `ai` run from an IDE still has speech recognition to reach on localhost.
* Qdrant is no longer tied to `rag`. A bundle that names a `qdrant` image now gets a store whether or not it has `rag`, and anything that needs one only has to declare a dependency on it. The store is stopped and starts on command, exactly as above. Community stands are unaffected: no community bundle names a qdrant image, and a bundle that declares `rag` without one still gets its store as before.
